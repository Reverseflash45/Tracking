...
class RestDayRepository {
  RestDayRepository(this._client);
  ...
  Future<List<RestDay>> fetchRestDays(String userId) async {
    ...
    return (rows as List)
        .map((row) => RestDay.fromMap(row as Map<String, dynamic>))
        .toList();
  }
...
final restDayRepositoryProvider = Provider<RestDayRepository>((ref) {
  return RestDayRepository(ref.watch(supabaseClientProvider));
});
